...
########################
# 1. 로직 버그 수정
# 2. 로직 개선 (정확도 향상 유무 판단 필요)
# -  대표 텍스트 생성 후 예제 지우고 대표 텍스트랑 비교 후 다시 추가 하는 방식임
########################
...
class FallInWord() :
    ...
    def run(self):
        ...
        for i, text in enumerate(self.before_texts):
            ...
            for cluster in tmp_clusters :
                ...
                conv_ratio = fuzz.token_set_ratio(cluster['totalText'], convert_text)
                if this_ratio >= 70 or conv_ratio >= 75 :
                    cluster['texts'].append(text)
                    text_test.append(text)
                    break
        print(len(self.before_texts), len(list(set(self.before_texts))), len(text_test))
        ...
        if len(self.texts) > 0 :
            for text in self.texts :
                if noun :
                    ...
                    self.ratio(original=text, text=text)
    ...
    def ratio(self, original, text):
        ...
        for i, ob in enumerate(self.clusterings) :
            this_ratio = fuzz.token_set_ratio(ob['totalText'], text)
            if max_ratio < this_ratio :
                max_ratio = this_ratio
                max_index = i
    ...
    def add_clustering(self, max_index, original, text):
        ...
        # count = Counter(re.split(' ', cluster['fullTotalText']))
        # sum = 0
        # i=0
        # for n, c in count.most_common(self.texts_len):
        #     i=i+1
        #     sum=sum+c
        # if len(re.split(' ', cluster['totalText'])) > self.texts_len and  sum/i < 2 :
        #     cluster['fullTotalText'] = cluster['fullTotalText'] + ' ' + text

        cluster['fullTotalText'] = cluster['fullTotalText'] + ' ' + text
        tmp_totalTexts = re.split(' ', cluster['fullTotalText'])
        set_totalTexts = list(set(re.split(' ', cluster['fullTotalText'])))

        if len(set_totalTexts) < self.texts_len :
            cluster['totalText'] = ' '.join(set_totalTexts)
        else :
